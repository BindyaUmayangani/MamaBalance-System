...
export async function resolveLinkedMidwifeUids(actor: UserProfile) {
  const uidSet = new Set<string>([actor.uid]);

  if (actor.role !== "midwife") {
    return [actor.uid];
  }
...
export async function loadMidwifeMothersByLinkedUids(midwifeUids: string[]) {
  const snapshots = await Promise.all(
    chunk(midwifeUids, 10).map((uids) =>
...
export async function loadMidwifeCollectionByLinkedUids(
