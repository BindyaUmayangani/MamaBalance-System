...
function calculateAge(value: unknown) {
  ...
  const hasHadBirthday = monthDelta > 0 || (monthDelta === 0 && today.getDate() >= birthdate.getDate());
...
async function loadRegionMap() {
  ...
  const regions = snapshot.empty
    ? DEFAULT_REGIONS
    : snapshot.docs.map((doc) => ({ id: doc.id, name: String(doc.data().name || doc.id) }));
...
async function loadStaffNameMap(uids: Set<string>) {
  if (uids.size === 0) return new Map<string, string>();
  const snapshots = await Promise.all(Array.from(uids).map((staffUid) => adminDb.collection("users").doc(staffUid).get()));
...
function assessmentDate(assessment: DocumentData) {
  return toDate(assessment.submittedAt ?? assessment.createdAt ?? assessment.attemptedAt ?? assessment.updatedAt);
}
...
function matchesMotherRecord(record: DocumentData, uid: string, userUid: string, motherUserId: string) {
  ...
  return motherId === uid || motherUid === uid || motherUid === userUid || recordUserId === motherUserId;
}
...
function formatObservedBy(data: DocumentData, source: ObservationSource, staffMap: Map<string, string>) {
  if (source === "doctor") {
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  const [epdsSnapshot, careObservationSnapshot, midwifeObservationSnapshot, medicationSnapshot] = await Promise.all([
  ...
  const epdsHistory = epdsSnapshot.docs
    .map((doc) => {
      ...
      if (!matchesMotherRecord(assessment, uid, userUid, motherUserId)) return null;
  ...
  const observationDocs = [
    ...careObservationSnapshot.docs.map((doc) => ({ id: doc.id, data: doc.data(), fallbackSource: "doctor" as ObservationSource })),
    ...midwifeObservationSnapshot.docs.map((doc) => ({ id: doc.id, data: doc.data(), fallbackSource: "homeVisit" as ObservationSource })),
  ...
  const medications = medicationDocs
    .map(({ id, data }) => {
      const prescribedByUid = String(data.prescribedByUid || "");
      const prescribedBy = staffMap.get(prescribedByUid) || String(data.prescribedBy || "Unknown Doctor");
