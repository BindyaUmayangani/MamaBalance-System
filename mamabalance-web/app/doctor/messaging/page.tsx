...
export default function MessagingPage() {
  ...
  async function loadMessages(conversationId = activeChatId) {
    ...
    const response = await fetch(`/api/doctor/messaging${suffix}`, { cache: "no-store" });
  ...
  async function sendMessage(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/doctor/messaging", {
        method: "POST",
          ...
          <div className="chat-thread" ref={threadRef}>
            ...
            {messages.map((message) => {
              ...
              return (
                <div
                  key={message.id}
                  className={isDoctor ? "message-row outgoing" : "message-row incoming"}
                >
                  {!isDoctor && <div className="message-badge">{motherInitial}</div>}

                  <div className="message-group">
                    <div className={isDoctor ? "message-bubble outgoing" : "message-bubble incoming"}>
                      {message.text}
                    </div>
                    <span className={isDoctor ? "message-time outgoing" : "message-time incoming"}>
                      {formatTime(message.createdAt)}
                    </span>
                  </div>

                {isDoctor && <div className="message-badge doctor">{staffInitial}</div>}
              </div>
              );
            })}
          </div>
